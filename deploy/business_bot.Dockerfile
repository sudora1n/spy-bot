...
RUN --mount=type=cache,target=/go/pkg/mod/ \
    ...
    cd business_bot && go mod download -x

COPY --from=buf-gen /workspace/business_bot/pb ./business_bot/pb

RUN --mount=type=cache,target=/go/pkg/mod/ \
    --mount=type=bind,target=. \
    CGO_ENABLED=0 go build -ldflags='-s -w -extldflags "-static"' -o /bin/business_bot ./business_bot/cmd/main.go
...
COPY --from=builder /bin/business_bot ./
