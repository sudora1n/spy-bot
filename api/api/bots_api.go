...
type BotsServer struct {
	...
	business managerv1connect.ManagerServiceClient
}

func NewBotsServer(mongo *repository.MongoRepository, business managerv1connect.ManagerServiceClient) *BotsServer {
	return &BotsServer{
		mongo:    mongo,
		business: business,
	}
...
func (s *BotsServer) CreateBot(
	...
	if botsLen > config.Config.MaxBotsByUser {
		return nil, connect.NewError(connect.CodeResourceExhausted, errors.ERROR_TOO_MANY_BOTS)
	}
	...
	if botExists != nil {
		return nil, connect.NewError(connect.CodeAlreadyExists, errors.ERROR_ALREADY_EXISTS)
	}

	newBot, err := telego.NewBot(req.Msg.Token, telego.WithAPIServer(config.Config.TelegramBot.ApiURL))
	if err != nil {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.ERROR_INVALID_TOKEN)
	}
	...
	if !botUser.CanConnectToBusiness || !botUser.SupportsInlineQueries {
		...
		err := connect.NewError(
			...
			errors.ERROR_INCORRECT_SETTINGS,
		)
	...
	_, err = s.business.CreateBot(ctx, connect.NewRequest(&managerv1.CreateBotRequest{Id: botUser.ID}))
	if err != nil {
		return nil, errors.CONNECT_ERROR_INTERNAL
	}
...
func (s *BotsServer) RemoveBot(
	...
	if bot.UserID != req.Msg.UserId {
		return nil, connect.NewError(connect.CodePermissionDenied, errors.ERROR_PERMISSION_DENIED)
	}

	_, err = s.business.RemoveBot(ctx, connect.NewRequest(&managerv1.RemoveBotRequest{Id: req.Msg.BotId}))
	if err != nil {
		return nil, errors.CONNECT_ERROR_INTERNAL
	}
