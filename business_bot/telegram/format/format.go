...
import (
	...
	"ssuspy-bot/consts"
	"ssuspy-bot/types"
	commonFormat "ssuspy-common/telegram/format"
...
func getForwardInfo(msg *telego.Message, loc *i18n.Localizer) string {
	...
	case *telego.MessageOriginUser:
		u := origin.SenderUser
		name := commonFormat.Name(u.FirstName, u.LastName)
		if u.Username != "" {
...
func EditedDiff(oldMsg *telego.Message, newMsg *telego.Message, loc *i18n.Localizer, truncate bool) ([]string, types.MediaDiff) {
	...
	oldMedia := commonUtils.GetFile(oldMsg)
	newMedia := commonUtils.GetFile(newMsg)
...
func FilterMessagesByDate(msgs []*telego.Message) []*telego.Message {
	...
	for _, m := range msgs {
		...
		if exist, ok := uniq[id]; !ok {
			...
			tNew := commonFormat.ChooseTime(m.EditDate, m.Date)
			tOld := commonFormat.ChooseTime(exist.EditDate, exist.Date)
...
func formatText(text string, truncate bool) string {
	if truncate {
		text = commonFormat.TruncateText(text, consts.MAX_MESSAGE_TEXT_LEN, true)
	}
